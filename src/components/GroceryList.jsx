...
export function GroceryList({ groceryList, setGroceryList, checkedItems, setCheckedItems, grocerySource, pantryItems, storeRouteInfo, setStoreRouteInfo, onRegenerate, onBuildAiList, onBuildQuickList, plannedCount, groceryBuilding }) {
  ...
  return (
    <div>
      <h2 style={{margin:"0 0 20px",fontSize:18,fontWeight:700,color:"#0C2C55"}}>🛒 Grocery List</h2>

      {/* Build buttons — always visible at top */}
      <div style={{background:"white",border:"1.5px solid #629FAD",borderRadius:14,padding:"16px",marginBottom:20,boxShadow:"0 2px 8px rgba(12,44,85,0.08)"}}>
        <div style={{fontWeight:700,fontSize:14,color:"#0C2C55",marginBottom:4}}>Build your list</div>
        <div style={{fontSize:12.5,color:"#629FAD",marginBottom:14}}>
          {plannedCount>0?`${plannedCount} meal${plannedCount>1?"s":""} planned${pantryItems.length>0?` · ${pantryItems.length} pantry items will be excluded`:""}`:
          "Plan meals in the Week tab first, then build your list here."}
        </div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <button onClick={onBuildAiList} disabled={!plannedCount||groceryBuilding} style={{background:plannedCount&&!groceryBuilding?"#0C2C55":"#a8c4cc",color:"white",border:"none",borderRadius:8,padding:"10px 18px",fontWeight:700,cursor:plannedCount&&!groceryBuilding?"pointer":"default",fontSize:14}}>
            {groceryBuilding?"⏳ Building...":"✨ AI List (with quantities)"}
          </button>
          <button onClick={onBuildQuickList} disabled={!plannedCount||groceryBuilding} style={{background:plannedCount&&!groceryBuilding?"white":"#f0f5f6",color:plannedCount?"#0C2C55":"#a8c4cc",border:`1.5px solid ${plannedCount?"#629FAD":"#a8c4cc"}`,borderRadius:8,padding:"10px 16px",fontWeight:600,cursor:plannedCount&&!groceryBuilding?"pointer":"default",fontSize:13}}>
            Quick List
          </button>
        </div>
      </div>
      ...
      {total>0&&<>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
          {storeRouteInfo&&<span style={{fontSize:13,fontWeight:500,color:"#629FAD"}}>🗺 {storeRouteInfo.storeName}</span>}
          <div style={{display:"flex",gap:8,marginLeft:"auto"}}>
            <button onClick={()=>setCheckedItems({})} style={{background:"white",color:"#0C2C55",border:"1.5px solid #629FAD",borderRadius:8,padding:"7px 12px",fontSize:13,fontWeight:600,cursor:"pointer"}}>Uncheck All</button>
            <button onClick={copyList} style={{background:"#0C2C55",color:"white",border:"none",borderRadius:8,padding:"7px 12px",fontSize:13,fontWeight:600,cursor:"pointer"}}>{cm||"📋 Copy"}</button>
          </div>
        </div>
