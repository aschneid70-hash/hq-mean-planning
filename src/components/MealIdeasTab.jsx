...
export function MealIdeasTab() {
  ...
  const saveCustomMeal=()=>{
    ...
    setName('');setDesc('');setTime('30');setIngredients('');setIsVeg(false);setIsGF(false);setIsGFA(false)
    setShowForm(false)
  }

  const deleteCustomMeal=(mealName)=>{
    const updated=customMeals.filter(m=>m.name!==mealName)
    setDoc(CUSTOM_DOC,{meals:updated})
  }
  ...
  const inputStyle={width:"100%",padding:"9px 12px",borderRadius:8,border:"1.5px solid #C8D8D6",fontSize:13.5,outline:"none",color:"#335765",background:"white",boxSizing:"border-box"}

  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <h2 style={{margin:0,fontSize:18,fontWeight:700,color:"#335765"}}>Meal Ideas</h2>
        <button onClick={()=>setShowForm(!showForm)} style={{background:showForm?"#f0e6e0":"#335765",color:showForm?"#7F543D":"white",border:"none",borderRadius:8,padding:"8px 14px",fontSize:13,fontWeight:600,cursor:"pointer"}}>{showForm?"Cancel":"+ Add Meal"}</button>
      </div>

      {showForm&&(
        <div style={{background:"white",border:"1.5px solid #B6D9E0",borderRadius:14,padding:"16px",marginBottom:16,boxShadow:"0 2px 8px rgba(51,87,101,0.08)"}}>
          <div style={{fontWeight:700,fontSize:15,color:"#335765",marginBottom:12}}>Add a New Meal</div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {[["🌱 Vegetarian",isVeg,setIsVeg],["🌾 Gluten-Free",isGF,setIsGF],["⚡ GF Adaptable",isGFA,setIsGFA]].map(([label,val,set])=>(
                <button key={label} onClick={()=>set(!val)} style={{background:val?"#335765":"white",color:val?"white":"#335765",border:`1.5px solid ${val?"#335765":"#C8D8D6"}`,borderRadius:99,padding:"6px 12px",fontSize:12,fontWeight:600,cursor:"pointer"}}>{label}</button>
              ))}
            </div>
            <button onClick={saveCustomMeal} disabled={!name.trim()} style={{background:name.trim()?"#335765":"#C8D8D6",color:"white",border:"none",borderRadius:8,padding:"10px",fontWeight:700,cursor:name.trim()?"pointer":"default",fontSize:14}}>Save Meal</button>
          </div>
        </div>
      )}
      ...
      <div style={{fontSize:12.5,color:"#74A8A4",marginBottom:12}}>{filtered.length} meals {customMeals.length>0&&`· ${customMeals.length} custom`}</div>
