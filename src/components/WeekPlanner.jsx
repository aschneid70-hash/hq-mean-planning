  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}>
        <h2 style={{margin:0,fontSize:18,fontWeight:700,color:"#335765"}}>Dinners</h2>
      ...
      {/* Week toggle */}
      <div style={{display:"flex",gap:0,marginBottom:16,background:"#DBE2DC",borderRadius:10,padding:3,width:"fit-content"}}>
        <button onClick={()=>{setWeekOffset(0);setAddingTo(null)}} style={{padding:"7px 18px",borderRadius:8,border:"none",background:weekOffset===0?"#335765":"transparent",color:weekOffset===0?"white":"#335765",fontWeight:600,fontSize:13,cursor:"pointer",transition:"all 0.15s"}}>This Week</button>
        <button onClick={()=>{setWeekOffset(1);setAddingTo(null)}} style={{padding:"7px 18px",borderRadius:8,border:"none",background:weekOffset===1?"#335765":"transparent",color:weekOffset===1?"white":"#335765",fontWeight:600,fontSize:13,cursor:"pointer",transition:"all 0.15s"}}>Next Week</button>
      </div>
      ...
      {days.map(day=>(
        <div key={day.key} style={{background:"white",border:"1.5px solid #C8D8D6",borderRadius:14,padding:"14px 16px",marginBottom:10,boxShadow:"0 1px 4px rgba(51,87,101,0.06)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontWeight:700,color:"#335765",fontSize:15}}>{day.label}</span>
              <span style={{fontSize:12,color:"#74A8A4"}}>{day.date}</span>
            </div>
            <button onClick={()=>setAddingTo(addingTo===day.key?null:day.key)} style={{background:"#DBE2DC",border:"none",borderRadius:8,padding:"5px 11px",fontSize:12,fontWeight:600,cursor:"pointer",color:"#335765"}}>{addingTo===day.key?"Close":weekPlan[day.key]?"Change":"+ Pick Meal"}</button>
          </div>
          {weekPlan[day.key]?<div style={{marginTop:8}}><MealCard meal={weekPlan[day.key]} onRemove={()=>setWeekPlan(p=>{const n={...p};delete n[day.key];return n})} inPlan compact/></div>:<div style={{fontSize:13,color:"#a0bab8",marginTop:4}}>Nothing planned yet</div>}
          {addingTo===day.key&&<div style={{marginTop:10,borderTop:"1px solid #DBE2DC",paddingTop:10}}><div style={{fontSize:12,color:"#74A8A4",marginBottom:8,fontWeight:600}}>Pick a meal:</div><div style={{maxHeight:280,overflowY:"auto"}}>{MEAL_IDEAS.map(m=><MealCard key={m.name} meal={m} onAdd={(meal)=>{setWeekPlan(p=>({...p,[day.key]:meal}));setAddingTo(null)}} compact/>)}</div></div>}
        </div>
      ))}
      ...
      {plannedCount>0&&(
        <div style={{background:"#e4eeec",border:"1.5px solid #74A8A4",borderRadius:14,padding:"16px",marginTop:4}}>
          ...
          <div style={{fontSize:13,color:"#4a6a6a",marginBottom:12}}>{plannedCount} meal{plannedCount>1?"s":""} planned for {weekOffset===0?"this":"next"} week.</div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
