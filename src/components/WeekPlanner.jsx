...
export function WeekPlanner({ weekPlan, setWeekPlan, pantryItems, onBuildAiList, onBuildQuickList, switchToGrocery }) {
  ...
  return (
    <div>
      ...
      {DAYS.map(day=>(
        <div key={day.label} style={{background:"white",border:"1.5px solid #C8D8D6",borderRadius:14,padding:"14px 16px",marginBottom:10,boxShadow:"0 1px 4px rgba(51,87,101,0.06)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <button onClick={()=>setAddingTo(addingTo===day.label?null:day.label)} style={{background:"#DBE2DC",border:"none",borderRadius:8,padding:"5px 11px",fontSize:12,fontWeight:600,cursor:"pointer",color:"#335765"}}>{addingTo===day.label?"Close":weekPlan[day.label]?"Change":"+ Pick Meal"}</button>
          </div>
          {weekPlan[day.label]?<div style={{marginTop:8}}><MealCard meal={weekPlan[day.label]} onRemove={()=>setWeekPlan(p=>{const n={...p};delete n[day.label];return n})} inPlan compact/></div>:<div style={{fontSize:13,color:"#a0bab8",marginTop:4}}>Nothing planned yet</div>}
          {addingTo===day.label&&<div style={{marginTop:10,borderTop:"1px solid #DBE2DC",paddingTop:10}}><div style={{fontSize:12,color:"#74A8A4",marginBottom:8,fontWeight:600}}>Pick a meal:</div><div style={{maxHeight:280,overflowY:"auto"}}>{MEAL_IDEAS.map(m=><MealCard key={m.name} meal={m} onAdd={(meal)=>{setWeekPlan(p=>({...p,[day.label]:meal}));setAddingTo(null)}} compact/>)}</div></div>}
        </div>
      ))}
      {plannedCount>0&&(
