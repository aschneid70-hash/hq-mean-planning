...
export function MealIdeasTab() {
  ...
  const filtered = MEAL_IDEAS.filter(m => {
    if (filterVeg && !m.tags.includes("vegetarian")) return false
    if (filterGF  && !m.tags.includes("gluten-free")) return false
  ...
  const filterBtn = (active, onClick, label) => (
    <button onClick={onClick} style={{
      background: active ? "#335765" : "white",
      color: active ? "white" : "#335765",
      border: `1.5px solid ${active ? "#335765" : "#C8D8D6"}`,
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {filterBtn(filterVeg, () => setFilterVeg(!filterVeg), "🌱 Vegetarian only")}
        {filterBtn(filterGF,  () => setFilterGF(!filterGF),   "🌾 GF only")}
      </div>
      <div style={{ fontSize: 12.5, color: "#74A8A4", marginBottom: 12, letterSpacing: "0.3px" }}>{filtered.length} meals</div>
