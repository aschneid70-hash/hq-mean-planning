...
const SESSION_KEY = 'hq-meal-unlocked'
...
export default function App() {
  ...
  const handleUnlock = () => { sessionStorage.setItem(SESSION_KEY, '1'); setUnlocked(true) }
  ...
  const buildQuickList = () => {
    ...
    const cat = {}
    filtered.forEach(item => { const c = categorizeIngredient(item.name); if (!cat[c]) cat[c] = []; cat[c].push(item) })
  ...
  const buildAiList = async () => {
    const meals = Object.values(weekPlan)
    if (!meals.length) return
    setGroceryLoading(true); setGroceryList({})
    const mealNames  = meals.map(m => m.name).join(', ')
    const pantryNote = pantryItems.length ? `Family already has: ${pantryItems.join(', ')}. Do NOT include these.` : ''
    try {
      const parsed = await askClaudeJSON(
        `Grocery list for family of 5 (2 adults, 3 kids 4/2/2) for: ${mealNames}. ${pantryNote}
Keys = category, values = ingredient strings with quantities (e.g. "2 lbs ground beef").
Categories: Produce, Dairy & Eggs, Meat & Seafood, Pantry & Dry Goods, Sauces & Condiments, Spices & Seasonings, Canned & Packaged. Only include used categories. Deduplicate. ONLY valid JSON.`
      )
      const cat = {}
      for (const [c, items] of Object.entries(parsed)) cat[c] = items.map(name => ({ name, sources: [] }))
      setGroceryList(cat); setCheckedItems({}); setStoreRouteInfo(null)
      setGrocerySource(`AI list for ${meals.length} meals: ${mealNames}`)
    } catch { buildQuickList() }
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#F5F0EB" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ fontSize: 13, color: "#74A8A4", letterSpacing: "0.5px" }}>Loading your family's data...</div>
      </div>
    </div>
  )
  ...
  const tabStyle = (t) => ({
    ...
    background: tab === t ? "#F5F0EB" : "transparent",
    color: tab === t ? "#335765" : "rgba(245,240,235,0.75)",
  ...
  return (
    <div style={{ fontFamily: "'Georgia', serif", minHeight: "100vh", background: "#F5F0EB", paddingBottom: 56 }}>
        ...
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
            <div>
              ...
              <div style={{ fontSize: 11.5, color: "#B6D9E0", marginTop: 4, letterSpacing: "0.5px" }}>
                5 people · Wife: vegetarian 🌱 · You & oldest: celiac 🌾
              </div>
            </div>
            <button
              onClick={() => { sessionStorage.removeItem(SESSION_KEY); setUnlocked(false) }}
              style={{ background: "rgba(182,217,224,0.15)", color: "#B6D9E0", border: "1px solid rgba(182,217,224,0.25)", borderRadius: 8, padding: "6px 12px", fontSize: 12, cursor: "pointer", letterSpacing: "0.3px" }}
            >🔒 Lock</button>
          </div>
          {/* Tab bar */}
          <div style={{ display: "flex", gap: 2, background: "rgba(51,87,101,0.5)", borderRadius: 99, padding: 3, overflowX: "auto", width: "fit-content", maxWidth: "100%", border: "1px solid rgba(182,217,224,0.2)" }}>
            {[
              ...
              ["pantry",  "🧺 Pantry"],
              ["ideas",   "💡 Ideas"],
              ["grocery", `🛒 Grocery${totalItems > 0 ? ` (${checkedCount}/${totalItems})` : ''}`],
            ].map(([key, label]) => (
      ...
      <div style={{ maxWidth: 680, margin: "24px auto 0", padding: "0 16px" }}>
        ...
        {!(groceryLoading && tab === 'grocery') && (
          <>
            {tab === 'planner' && <WeekPlanner weekPlan={weekPlan} setWeekPlan={setWeekPlan} pantryItems={pantryItems} onBuildAiList={buildAiList} onBuildQuickList={buildQuickList} switchToGrocery={() => setTab('grocery')} />}
            {tab === 'pantry'  && <PantryTab pantryItems={pantryItems} setPantryItems={setPantryItems} />}
            {tab === 'ideas'   && <MealIdeasTab />}
            {tab === 'grocery' && <GroceryList groceryList={groceryList} setGroceryList={setGroceryList} checkedItems={checkedItems} setCheckedItems={setCheckedItems} grocerySource={grocerySource} pantryItems={pantryItems} storeRouteInfo={storeRouteInfo} setStoreRouteInfo={setStoreRouteInfo} onRegenerate={buildAiList} plannedCount={plannedCount} />}
          </>
        )}
      </div>
    </div>
